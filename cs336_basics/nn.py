...
class Swiglu(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        value = self.w3(x)
        hidden = gate * torch.sigmoid(gate) * value
...
class Scaled_dot_product_attention(torch.nn.Module):
    ...
    def forward(self, Q: torch.Tensor, K: torch.Tensor, V: torch.Tensor, mask: torch.Tensor | None = None) -> torch.Tensor:
        ...
        attn_weights = softmax(scores, dim=-1)  # 对分数进行 softmax，得到注意力权重
...
def softmax(x: Tensor, dim: int = -1) -> Tensor:
    ...
    x_max = x.max(dim=dim, keepdim=True).values #沿指定 dim 找最大值, 取max会变成张量，keepdim=True 保留该维度，方便广播
